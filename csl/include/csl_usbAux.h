...
void USB_dmaTxStart(DWORD    dwEndpoint)
{
	...
	/* clear auticlear (bit 15) and DMA mode (bit 11) */
                        ...
                        DWORD           dwEndpoint)
{
    ...
    if(pContext != NULL)
    {
		...
		if((pTransfer != NULL) && (pTransfer->fComplete == FALSE))
		{
			...
			cbToWrite = (pTransfer->cbBuffer - pTransfer->cbTransferred);
			pFifoAddr = (volatile ioport Uint16*)peps->pFifoAddr;

			/* Check if USB is configured for DMA */
			if((cbToWrite == 512) && (pContext->opMode == CSL_USB_OPMODE_DMA))
			{
			...
			else
			{
				...
				if(cbToWrite > 0)
				{
					...
					if(cbToWrite % 2)
					{
						...
						CSL_FINS(*pFifoAddr, USB_FIFO1R1_DATA, *pwBuffer);
						/* Disable byte access */
                        ...
                        DWORD           dwEndpoint)
{
	...
	if(pContext != NULL)
    {
		...
		if((pTransfer != NULL) && (pTransfer->fComplete == FALSE))
		{
			...
			pwBuffer = (WORD*)pTransfer->pvBuffer + pTransfer->cbTransferred/2;
			pFifoAddr   = (volatile ioport Uint16*)peps->pFifoAddr;
			bytesToRead = (Uint16)(pTransfer->cbBuffer - pTransfer->cbTransferred);
			/* To avoid compilation warning */
			bytesToRead = bytesToRead;

			if((bytesToRead == 512) && (pContext->opMode == CSL_USB_OPMODE_DMA))
			{
				USB_dmaRxStart(dwEndpoint);
				pContext->fWaitingOnFlagB = FALSE;
				pTransfer->cbTransferred = bytesToRead;
				status = pContext->completeTransferCallback(pContext, peps);
			}
			else
			{
				...
				dwBytesRead = 0;
				if(cbBuffer >= peps->dwPacketSizeAssigned)
				{
					while(cbBuffer >= peps->dwPacketSizeAssigned)
					{
						/* read all data */
						for(looper = 0; looper < peps->dwPacketSizeAssigned/2; looper++)
						{
							*pwBuffer++ = *pFifoAddr;
						}

						dwBytesRead += peps->dwPacketSizeAssigned;
						cbBuffer -= peps->dwPacketSizeAssigned;
					}
					pTransfer->cbTransferred += dwBytesRead;
				}
				/* check if we have a short or null packet */
				else
				{
					/* short packet */
					if(cbBuffer !=0 && cbBuffer < peps->dwPacketSizeAssigned)
					{
						for(looper = 0; looper < (cbBuffer+1)/2; looper++)
						{
							*pwBuffer++ = *pFifoAddr;
						}

						pTransfer->cbTransferred += cbBuffer;
						pTransfer->fComplete = TRUE;
					}
					/* null packet */
					else
...
#endif
				status = pContext->completeTransferCallback(pContext, peps);
				for(looper = 0;looper < CSL_USB_DATA_DELAY;looper++);
			}
		}
		else
		{
			  		...
			  		CSL_UsbBusSpeed    *pBusSpeed)
{
    ...
    if ((usbRegisters->FADDR_POWER) & ~(CSL_USB_FADDR_POWER_HSMODE_MASK))
    {
                          ...
                          Uint16           confVal)
{
	...
	/* Flush FIFO if not empty */
    if((usbRegisters->PERI_RXCSR_INDX & CSL_USB_PERI_RXCSR_FIFOFULL_MASK) ==
       CSL_USB_PERI_RXCSR_FIFOFULL_MASK)
	{
